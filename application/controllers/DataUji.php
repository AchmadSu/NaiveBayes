...
class DataUji extends CI_Controller
{
	...
	function hitung(){
		...
		else
		{
			...
			// $rw = $this->Training_Model->rw($this->input->post('rw'));
  ...
  //perhitungan //Step 1
   ...
   //tampil
			...
			$kelas_layak = round($jumlah_tanggungan['layak'], 2)*round($jml_penghasilan['layak'], 2)*$PC1;

			$kelas_tidak_layak = round($jumlah_tanggungan['tidaklayak'], 2)*round($jml_penghasilan['tidaklayak'], 2)*$PC0;
   ...
   //step 3
			...
			// $output .= "<br>Kondisi Rumah: ".round($rw['tidaklayak'], 2);
			...
			// $output .= $kelas_tidak_layak = round($status_PKH['tidaklayak'],2)*round($jumlah_tanggungan['tidaklayak'], 2)*round($kepala_rt['tidaklayak'], 2)*round($rw['tidaklayak'], 2)*round($jml_penghasilan['tidaklayak'], 2)*round($status_rumah['tidaklayak'], 2)*$PC0;
			...
			// $output .= "<br>Kondisi Rumah: ".round($rw['layak'], 2);
			// $output .= "<br>Jumlah Penghasilan: ".round($jml_penghasilan['layak'], 2);
			// $output .= "<br>Status Rumah: ".round($status_rumah['layak'], 2);
			// $output .= "<br> Hasil Probabilitas: ";
			// $output .= $kelas_layak = round($status_PKH['layak'],2)*round($jumlah_tanggungan['layak'], 2)*round($kepala_rt['layak'], 2)*round($rw['layak'], 2)*round($jml_penghasilan['layak'], 2)*round($status_rumah['layak'], 2)*$PC1;
			...
			if ($this->input->post('pkh') == '1') {
				$output .= "<br>Dapat disimpulkan Bahwa Data Uji tersebut <b><u>Tidak Layak (PKH).</u></b> Untuk menerima Bantuan BLT Desa Bojong";
			} else if($this->input->post('pkh') == '0' && ($this->input->post('jml_penghasilan')/$this->input->post('jml_tanggungan')<= 1000000)) {
				$output .= "<br>Dapat disimpulkan Bahwa Data Uji tersebut <b><u>Layak</u></b> Untuk menerima Bantuan BLT Desa Bojong";
			} else {
				$output .= "<br>Dapat disimpulkan Bahwa Data Uji tersebut <b><u>".$kesimpulan."</u></b> Untuk menerima Bantuan BLT Desa Bojong";
			}
